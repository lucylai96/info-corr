...
elseif processed==3 % choose a dimensionality, and reduce either neurons or trials and look at FI.
    load('j1_171018_estFI.mat')
    bs_TFI_N = zeros(50,6);
    err_TFI_N = zeros(50,6);
    bs_TFI_T = zeros(50,6);
    err_TFI_T = zeros(50,6);
    TFI_N = zeros(50,6);
    TFI_T = zeros(50,6);
...
%   
    ...
    for dim = 10:10:400 %for these dimensionalities
        %% do by cutting off NEURONS
        t = 800; %use all the trials
        n = dim; %only use "dim" # of neurons
        for b  = 1:50 %bootstrap 100 times
            rp1=randperm(800);
            rp2=randperm(500);
            ...
            for i  = 1:length(orient)-1 %comparisons are # of orientations - 1
                ...
                if t>(n+2)/2 %if you have more trials than neurons
                    TFI_N(b,i) = f_info(t_highConRespOri_bsN(:,:,i), t_highConRespOri_bsN(:,:,i+1),orient(i),orient(i+1));%this is the bias-corrected FI in units of rad^-2
                else %less trials than neurons
                    TFI_N(b,i) = ft_info(t_highConRespOri_bsN(:,:,i), t_highConRespOri_bsN(:,:,i+1),orient(i),orient(i+1));%this is the bias-corrected FI in units of rad^-2
                end
            end
        end
        %each row is a dimension (# neurons)
        bs_TFI_N(dim/10,:) = mean(TFI_N,1); %here is taking mean across all conditions (should also do for just one condition at a time)
        err_TFI_N(dim/10,:) = sem(TFI_N,1);
        
        
        %figure;shadedErrorBar([10:10:500]',mean(bs_TFI_N,2),std(bs_TFI_N,[],2),[],1)
        
        %% do by cutting off TRIALS
        t = dim*2;  %has to be t<(n+2)/2, new dim is K = 2T-2, only use "dim" # of trials
        ...
        for b  = 1:50 %bootstrap 100 times
            ...
            for i  = 1:length(orient)
                ...
                t_highConRespOri_bsTrials(:,:,i) = highConRespOri(rp1(1:t),:,i);%datasample(highConRespOri(:,:,i),t,1, 'Replace', false); % 3rd param: 1=subsampling trials, 2=subsampling neurons
                % bootstrapping to get a reduced matrix of n neurons
                t_highConRespOri_bsT(:,:,i)= t_highConRespOri_bsTrials(:,rp2(1:n),i);
            end
            ...
            for i  = 1:length(orient)-1 %comparisons are # of orientations - 1
                ...
                %conditions and the rows are the bootstraps
                if t>(n+2)/2 %if you have more trials than neurons
                    TFI_T(b,i) = f_info(t_highConRespOri_bsT(:,:,i), t_highConRespOri_bsT(:,:,i+1),orient(i),orient(i+1));%this is the bias-corrected FI in units of rad^-2
                else %less trials than neurons
                    TFI_T(b,i) = ft_info(t_highConRespOri_bsT(:,:,i), t_highConRespOri_bsT(:,:,i+1),orient(i),orient(i+1));%this is the bias-corrected FI in units of rad^-2
                end
            end
        end
        %each row is a dimension (# neurons)
        bs_TFI_T(dim/10,:) = mean(TFI_T,1); %here is taking mean across all conditions (should also do for just one condition at a time)
        err_TFI_T(dim/10,:) = sem(TFI_T,1);
...
xlabel('I_F (reduced dim using neurons');ylabel('I_F reduced dim using trials')
    ...
    figure;
    subplot 121
    shadedErrorBar([10:10:400]',mean(bs_TFI_N(1:40,:),2),std(bs_TFI_N(1:40,:),[],2),[],1)
    xlabel('# neurons');ylabel('I_F (rad^{-2})')
    title('# trials = 800')
prettyplot
    subplot 122;
    shadedErrorBar([10:10:400]*2',mean(bs_TFI_T(1:40,:),2),std(bs_TFI_T(1:40,:),[],2),[],1)
     xlabel('# trials');
         title('# neurons = 500')
     prettyplot
...
function [CTFI] = ft_info(r_s1,r_s2,s1,s2)
